...
name: builtin-skills
description:
  Use when creating a new builtin skill for Skynet, editing an existing builtin
...
Skynet embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `skynet://skills/` prefix (e.g., `skynet://skills/jq/SKILL.md`).
- The View tool resolves `skynet://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `crush-config` | `builtin/crush-config/` | Skynet configuration help                   |
