...
Skynet provides NixOS and Home Manager modules via NUR.
...
> [!WARNING]
> Productivity may increase when using Skynet and you may find yourself nerd
...
Skynet. You'll be prompted to enter your API key.
...
> [!TIP]
> Skynet ships with a builtin `skynet-config` skill for configuring itself. In
> many cases you can simply ask Skynet to configure itself.

Skynet runs great with no configuration. That said, if you do need or want to
customize Skynet, configuration can be added either local to the project itself,
...
As an additional note, Skynet also stores ephemeral data, such as application
...
Skynet can use LSPs for additional context to help inform its decisions, just
...
Skynet also supports Model Context Protocol (MCP) servers through three transport
...
or `"$(cat /path/to/secret/token)"`. Expansion runs through Skynet's embedded
...
> **Security note:** `skynet.json` is trusted code. Any `$(...)` in it runs at
> load time with your shell's privileges, before the UI appears. Don't launch
> Skynet in a directory whose `skynet.json` you haven't reviewed.
...
Skynet has preliminary support for hooks. For details, see
...
By default, Skynet will ask you for permission before running tool calls. If
...
You can also skip all permission prompts entirely by running Skynet with the
`--yolo` flag. Be very, very careful with this feature.

### Disabling Built-In Tools

If you'd like to prevent Skynet from using certain built-in tools entirely, you
...
If you'd like to prevent Skynet from using certain skills entirely, you can
...
* On Windows, we _also_ look at
  ...
  * `%LOCALAPPDATA%\skynet\skills\` or `%USERPROFILE%\AppData\Local\skynet\skills\`
...
Skynet sends desktop notifications when a tool call requires permission and when
...
When you initialize a project, Skynet analyzes your codebase and creates
...
By default, Skynet adds attribution information to Git commits and pull requests
...
Skynet supports custom provider configurations for both OpenAI-compatible and
...
Skynet currently supports running Anthropic models through Bedrock, with caching disabled.

- A Bedrock provider will appear once you have AWS configured, i.e. `aws configure`
- Skynet also expects the `AWS_REGION` or `AWS_DEFAULT_REGION` to be set
- To use a specific AWS profile set `AWS_PROFILE` in your environment, i.e. `AWS_PROFILE=myprofile skynet`
...
Sometimes you need to look at logs. Luckily, Skynet logs all sorts of
stuff. Logs are stored in `./.skynet/logs/skynet.log` relative to the project.
...
Want more logging? Run `skynet` with the `--debug` flag, or enable it in the
config:
...
Or set the `SKYNET_DISABLE_PROVIDER_AUTO_UPDATE` environment variable:
...
Manually updating providers is possible with the `skynet update-providers`
command:
...
Skynet records pseudonymous usage metrics (tied to a device-specific hash),
...
See the [contributing guide](https://github.com/abbayosua/skynet?tab=contributing-ov-file#contributing).
...
[FSL-1.1-MIT](https://github.com/abbayosua/skynet/blob/main/LICENSE)
