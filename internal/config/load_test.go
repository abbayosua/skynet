...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	t.Run("does not pick up skynet.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// skynet.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "skynet.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "skynet.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find skynet.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "skynet.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent skynet.json")
		}
	})

	t.Run("picks up skynet.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "skynet.json")
		...
		require.True(t, foundLocal, "expected project skynet.json to be in lookup result: %v", got)
	})
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "agent_status", "ast_grep_search", "ast_grep_replace", "bash", "collect_agent", "skynet_info", "skynet_logs", "job_output", "job_kill", "hashline_edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "planner", "sleep", "spawn_agent", "sourcegraph", "team", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "agent_status", "ast_grep_search", "ast_grep_replace", "bash", "collect_agent", "skynet_info", "skynet_logs", "job_output", "job_kill", "download", "edit", "hashline_edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "planner", "sleep", "spawn_agent", "team", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "skynet.json")
